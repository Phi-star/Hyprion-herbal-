...
app.get('/api/hyprion-images', (req, res) => {
    ...
    try {
        if (!fs.existsSync(folderPath)) {
            return res.json({ success: true, images: [], count: 0, folder: rawFolder, message: 'Folder not found' });
        }

        // Accept all common image extensions — case-insensitive
        // Covers .jpg .jpeg .png .PNG .JPG .webp .WEBP .gif .GIF .avif .bmp
        const allowed = ['.jpg', '.jpeg', '.png', '.webp', '.gif', '.avif', '.bmp', '.tiff', '.svg'];

        const files = fs.readdirSync(folderPath);
        const images = files
            .filter(f => {
                const ext = path.extname(f).toLowerCase(); // lowercase so .PNG matches .png
                return allowed.includes(ext);
            })
            .sort()  // alphabetical so order is predictable
            .map(f => `/${rawFolder}/${f}`);

        console.log(`📸 Image API: found ${images.length} image(s) in ${rawFolder}`);
