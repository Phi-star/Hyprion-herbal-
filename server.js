...
// ============================================================
// WWW REDIRECT
// non-www → www  (e.g. magdoraherbalwellness.name.ng → www.magdoraherbalwellness.name.ng)
// ============================================================
app.use((req, res, next) => {
    const host = req.headers.host || '';
    // Only redirect on the production domain, not on localhost or herokuapp.com
    if (
        host === 'magdoraherbalwellness.name.ng' ||
        host === 'magdoraherbalwellness.name.ng:443' ||
        host === 'magdoraherbalwellness.name.ng:80'
    ) {
        const redirectUrl = `https://www.magdoraherbalwellness.name.ng${req.originalUrl}`;
        console.log(`🔀 Redirecting non-www → www: ${host}${req.originalUrl}`);
...
// ============================================================
// API — EXTRA IMAGE PROXIES (bonus, delivery, nafdac, hypertension)
// All served through server to avoid ad-blocker / Brave issues
// ============================================================
const BONUS_IMAGES = [
    'https://files.catbox.moe/l4o3l3.jpg',  // bonus 1 - Stress & Sleep
    'https://files.catbox.moe/98epaf.jpg',  // bonus 2 - BP Tracking
    'https://files.catbox.moe/murlhw.jpg',  // bonus 3 - 7-Day Meal Plan
    'https://files.catbox.moe/m6vz2j.jpg'   // bonus 4 - Home Monitoring
];
const MISC_IMAGES = {
    delivery:       'https://files.catbox.moe/31t2ja.jpg',
    nafdac:         'https://files.catbox.moe/7thxdx.jpg',
    hypertension:   'https://files.catbox.moe/jdmeby.jpg'
};

app.get('/api/hyprion-bonus/:index', async (req, res) => {
    const idx = parseInt(req.params.index, 10);
    if (isNaN(idx) || idx < 0 || idx >= BONUS_IMAGES.length)
        return res.status(404).json({ error: 'Image not found' });
    try {
        const imgResp = await axios.get(BONUS_IMAGES[idx], {
        ...
        console.error(`Bonus image proxy error (${idx}):`, err.message);
        res.status(502).json({ error: 'Failed to fetch image' });
    }
});

app.get('/api/hyprion-misc/:name', async (req, res) => {
    const url = MISC_IMAGES[req.params.name];
    ...
    try {
        ...
        console.error(`Misc image proxy error (${req.params.name}):`, err.message);
        res.status(502).json({ error: 'Failed to fetch image' });
    }
});
...
app.post('/api/hyprion-order', orderRateLimit, async (req, res) => {
    try {
        const { fullName, phone, whatsapp, email, state, city, address, package: pkg, packageQty } = req.body;
